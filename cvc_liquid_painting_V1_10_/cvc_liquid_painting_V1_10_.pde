...
*/
  ...
  boolean CVC_DEBUG_RENDER           = false; // true
  
  PImage image;
    
  int viewport_w = 1200; //width
  int viewport_h = 769; //600 height
  ...
  int savedTime;
  int savedTime_2;
  int totalTime = 15000; // time in seconds (15000=15 seconds) 
...
void setup() {
    ...
    savedTime = millis();
    img[0] = loadImage("rug_0.jpg");
    img[1] = loadImage("rug_1.jpg");
    img[2] = loadImage("rug_2.jpg");
    img[3] = loadImage("rug_3.jpg");
    img[4] = loadImage("rug_4.jpg");
    img[5] = loadImage("rug_5.jpg");
    img[6] = loadImage("rug_6.jpg");
    img[7] = loadImage("rug_7.jpg");
    img[8] = loadImage("rug_8.jpg");
    img[9] = loadImage("rug_9.jpg");
...
// image/buffer that will be used as density input
    ...
    pg_image.image(image, 0, 0); // adds bg image into fluid engine
    pg_image.endDraw();
...
void draw() {
    ...
    if(UPDATE_FLUID){
      fluid.update();
      addDensityTexture(fluid, pg_image, fluid.simulation_step == 0 ? 1.0f : 0.01f);
    ...
    myChangeBgTexture(); // call the change bg texture function see bottom of code
...
void updateTrackingBlobs(TrackingEvent event) {
  ...
  //particles  
   ...
   //   if(mouseButton == LEFT){
        radius = random(0, 2000); //size of particle explosion!
        vscale = random(-2000, 2000); //velocity scale?
...
void keyPressed() {
  ...
  if(key == 'N' || key == 'n') myChangeBgTexture(); // call to change bg texture on the fly
...
//      pg_tex_handle[0] = pg.getTexture().glName;
 ...
 void myChangeBgTexture(){
  ...
  if (passedTime > totalTime) { // Has five seconds passed?
    ...
    //fluid_reset(); // included reset experiment removing...
   int index = int(random(0, img.length));  // random array
    
   index = index + 1;
  if (index < 10){
    //fluid_reset(); // included reset experiment removing...
    img[index].resize(width, height); //resize
    pg_image = (PGraphics2D) createGraphics(viewport_w, viewport_h, P2D);
    pg_image.noSmooth();
    pg_image.beginDraw();
    pg_image.clear();
    ...
    //pg_image.scale(viewport_h / (float)image.height);
    pg_image.imageMode(CENTER);
    pg_image.image(img[index], 0, 0);  // attempts to inject new bg into engine texture buffer
    myfadeIn();
    pg_image.endDraw();
  ...
  println("rug Number "+index);
 }
}

void myfadeIn(){
  int passedTime_2 = millis() - savedTime_2; // calculate time passes
  if (passedTime_2 > totalTime/10) { // Has five seconds passed?
  for (float i = 0; i < 255; i= i + 0.2){
    pg_image.tint(255, i);
    println("fadeIn"+i);
  }
 savedTime_2 = millis(); 
